...
public class AESModel {
	...
	/**
	 * mixColumns GF4 2^8 table
	 */
	public static final int[][] gf = {{0x02, 0x03, 0x01, 0x01},
										{0x01, 0x02, 0x03, 0x01},
										{0x01, 0x01, 0x02, 0x03},
										{0x03, 0x01, 0x01, 0x02}};

	/**
	 * Inverse of the mixColumns GF4 2^8 table 
	 * 
	 */
	public static final int[][] invgf = {{0x0e, 0x0b, 0x0d, 0x09},
										{0x09, 0x0e, 0x0b, 0x0d},
										{0x0d, 0x09, 0x0e, 0x0b},
										{0x0b, 0x0d, 0x09, 0x0e}};

	/** 
	 * table used in the keyExpansion method
	 */
	public static final int[] keyExpansionArray = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
...
//			String line = String.format("%040x", new BigInteger(1, lineInput.getBytes(Charset.forName("UTF-8"))));
	...
	/**
	 * Help to find the sub key in the table 
	 * used in the KeyExpansion method
	 * @param km
	 * @param begin
	 * @return tab 2D 
	 */
	...
	/**
	 * Change the values from the table with those from the S-BOX
	 * @param tab qui est le tableau d'entrée 
	 */
	public void subBytes(int[][] tab){
		for (int i = 0; i < tab.length; i++) {
			for (int j = 0; j < tab[0].length; j++) {
				int hex = tab[j][i];
				tab[j][i] = sbox[hex / 16][hex % 16];
			}
		}
	}

	/**
	 * Change the values from the table with those from the INV S-BOX
	 * @param tab qui est le tableau d'entré 
	 */
	public void invSubBytes(int[][] tab){
		for (int i = 0; i < tab.length; i++) {
			for (int j = 0; j < tab[0].length; j++) {
				int hex = tab[j][i];
				tab[j][i] = invsbox[hex / 16][hex % 16];
			}
		}
	}
	...
	public void shiftRows(int[][] tab){
		for (int i = 1; i < tab.length; i++) {
			tab[i] = leftrotate(tab[i], i);
		}
	}

	/**
	 * Rotate to the left each elements of the line
	 * @param tab : state 
	 * @param times : gives the number of times we use the left rotation
	 * @return
	 */
	private int[] leftrotate(int[] tab, int times){
		assert(tab.length == 4); // pas besoin, juste pour confirmer que length est égal à 4 
		if (times % 4 == 0) {
			return tab;
		}
		while (times > 0) {
	...
	public void invShiftRows(int[][] tab){
		for (int i = 1; i < tab.length; i++) {
			tab[i] = rightrotate(tab[i], i);
		}
	}

	/**
	 * Works like the inverse of leftrotate
	 * first test what's already on the line 
	 * then move every elements from one square
	 * @param tab
	 * @param times
	 * @return
	 */
	private int[] rightrotate(int[] tab, int times) {
		if (tab.length == 0 || tab.length == 1 || times % 4 == 0) {
			return tab;
		}
		while (times > 0) {
	 ...
	 * 
	 * @param in
	 * @param keyExpansionArrayPointer
	 * @return
	 */
	public int[] schedule_core(int[] in, int keyExpansionArrayPointer) {
		in = leftrotate(in, 1);
		int hex;
		...
		in[0] ^= keyExpansionArray[keyExpansionArrayPointer];
		return in;
	}

	/**
	 * ToString method that transform a matrix to a String  
	 * @param m
	 * @return
	 */
	...
	/**
	 * MixColumns arrays, taken on wikipedia
	 */
